        ...
        <div key={project.id} className="w-[90%] md:w-[70%] flex flex-col  gap-2 p-4  rounded-md shadow-md ">
          ...
          <div className="flex flex-wrap gap-2">
              {project.skill?.map((skill) => (
                <span key={skill} className=" w-fit h-fit flex items-center sm:text-lg text-base font-light bg-[#FF9D23] hover:text-[#FF9D23] hover:bg-white hover:font-medium transition-all px-2 py-1 rounded-full cursor-default">
                  {skill}
                </span>
              ))}
          </div>
